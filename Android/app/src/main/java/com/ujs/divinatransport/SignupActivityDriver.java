...
public class SignupActivityDriver extends AppCompatActivity {
        ...
        btn_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (btn_next.getText().equals("Submit")) {
                    if (car_price == 0) {
                        MyUtils.showAlert(SignupActivityDriver.this, getResources().getString(R.string.warning), getResources().getString(R.string.please_fill_in_blank_field));
                        index_step--;
                        return;
                    }
                    if (car_photo == null) {
                        MyUtils.showAlert(SignupActivityDriver.this, getResources().getString(R.string.warning), getResources().getString(R.string.please_select_a_photo));
                        index_step--;
                        return;
                    }
    ...
    private void switchStep(boolean isIncreasing) {
        ...
        switch (index_step) {
            ...
            case 2:
                if (isIncreasing) {
                    if (user_photo == null) {
                        MyUtils.showAlert(this, getResources().getString(R.string.warning), getResources().getString(R.string.please_select_a_photo));
                        index_step--;
                        return;
                    }
                    if (user_name.length()*user_email.length() == 0) {
                        MyUtils.showAlert(this, getResources().getString(R.string.warning), getResources().getString(R.string.please_fill_in_blank_field));
                        index_step--;
                        return;
                    }
                    if (!MyUtils.isValidEmail(user_email)) {
                        MyUtils.showAlert(this, getResources().getString(R.string.warning), getResources().getString(R.string.invalid_email));
                        index_step--;
                        return;
                    }
    ...
    public void uploadLicensePhotoToFirebase() {
        ...
        final StorageReference file_refer = MyUtils.mStorage.child(MyUtils.storage_driving_license+ts);
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                file_refer.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        DrivingLicense license = new DrivingLicense("", MyUtils.cur_user.uid, downloadUrl, license_first_name, license_last_name, license_gender, license_birth_date, license_issue_date, license_expiry_date, license_number);
                        MyUtils.mDatabase.child(MyUtils.tbl_driving_license).push().setValue(license);
                        MyUtils.cur_user.state = 1;
                        MyUtils.mDatabase.child(MyUtils.tbl_user).child(MyUtils.cur_user.uid).child("state").setValue(1);
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                file_refer.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        dismissProgress();
                        String downloadUrl = uri.toString();
                        Car car = new Car("", MyUtils.cur_user.uid, downloadUrl, car_type, car_seats, car_price, true);
                        MyUtils.mDatabase.child(MyUtils.tbl_car).push().setValue(car);
                        MyUtils.cur_user.state = 2;
                        MyUtils.mDatabase.child(MyUtils.tbl_user).child(MyUtils.cur_user.uid).child("state").setValue(2);
